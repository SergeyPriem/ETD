...
# Entities are defined at module level (the standard Pony idiom), so
# `from models import Users` no longer connects to the database — importing
# the module has no side effects. The bind + mapping happens once, lazily,
# via connect() (called at app start). This makes the whole module graph
# importable/testable without a live DB.
...
def _bind_db():
    """Bind to MySQL and generate the mapping exactly once per process."""
    set_sql_debug(False)
    db.bind(
        provider='mysql',
...
def connect():
    """Idempotent connect — safe to call on every Streamlit rerun."""
    _bind_db()
